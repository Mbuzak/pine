...
void Scene::Setup() {
  pipelines_.insert(std::pair<std::string, Pipeline*>("default", new Pipeline("vertex.glsl", "fragment.glsl")));
  pipelines_.insert(std::pair<std::string, Pipeline*>("skybox", new Pipeline("skybox-vertex.glsl", "skybox-fragment.glsl")));
...
void Scene::Display() {
  ...
  RenderShapes();
	...
	pipelines_.at("default")->Disactivate();
}

void Scene::RenderToTexture() {
  pipelines_.at("default")->Activate();

  glUniformMatrix4fv(glGetUniformLocation(pipelines_.at("default")->id_, "matProj"), 1, GL_FALSE, glm::value_ptr(_projection_matrix));
  glUniform1i(glGetUniformLocation(pipelines_.at("default")->id_, "isLight"), false);

  background_[1]->Display(pipelines_.at("default")->id_);
...
void Scene::RenderSkybox() {
  pipelines_.at("skybox")->Activate();

  skybox_->Draw(pipelines_.at("skybox")->id_, _projection_matrix, _view_matrix);
}
...
void Scene::RenderShapes() {
  ...
  SendLight();
  sun_->SendUniform(pipelines_.at("default")->id_);

  glUniformMatrix4fv(glGetUniformLocation(pipelines_.at("default")->id_, "matProj"), 1, GL_FALSE, glm::value_ptr(_projection_matrix));
  glUniform1i(glGetUniformLocation(pipelines_.at("default")->id_, "isLight"), false);

  // send camera
  camera_.SendUniform(pipelines_.at("default")->id_);

  // potok graficzny mapy cieni ?
  glUniformMatrix4fv(glGetUniformLocation(pipelines_.at("default")->id_, "lightProj"), 1, GL_FALSE, glm::value_ptr(lightProj));
  glUniformMatrix4fv(glGetUniformLocation(pipelines_.at("default")->id_, "lightView"), 1, GL_FALSE, glm::value_ptr(lightView));

  dir_shadow_map_->SendTexture(pipelines_.at("default")->id_);

  // rysowanie obiektów nie-selekcyjnych (identyfikator 0)
  glStencilFunc(GL_ALWAYS, 0, 0xFF);

  for (Shape *shape : background_)
    shape->Display(pipelines_.at("default")->id_);
  
  game_->Display(pipelines_.at("default")->id_);
}

void Scene::RenderLights() {
  pipelines_.at("default")->Activate();
  glUniform1i(glGetUniformLocation(pipelines_.at("default")->id_, "isLight"), true);

  for (int i = 0; i < 4; i++) {
    glUniform3fv(glGetUniformLocation( pipelines_.at("default")->id_, "lightModelColor" ), 1, &lamps_[i]->diffuse[0]);
    lamps_[i]->Display(pipelines_.at("default")->id_);
  }
}
